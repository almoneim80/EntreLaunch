...
public class ContactScheduledEmailTask : BaseTask
{
            ...
            foreach (var schedule in schedules)
            {
                try
                {
                    ...
                    if (lastEmailLog is not null && lastEmailLog!.Status is EmailStatus.NotSent)
                    {
                        ...
                        {
                            nextEmailTemplateToSend = GetNextEmailTemplateToSend(lastEmailTemplate, schedule.Schedule!.GroEntreLaunchId);
                        }
                        else
                        {
    ...
    private EmailTemplate? GetNextEmailTemplateToSend(EmailTemplate lastEmailTemplate, int groEntreLaunchId)
    {
        var emailsOfGroEntreLaunch = dbContext.EmailTemplates!.Where(t => t.EmailGroEntreLaunchId == groEntreLaunchId).OrderBy(t => t.Id).ToList();

        var indexOfLastEmail = lastEmailTemplate is not null
            ? emailsOfGroEntreLaunch.IndexOf(lastEmailTemplate)
            : -1;

        if (emailsOfGroEntreLaunch.Count == indexOfLastEmail + 1)
        {
            return null;
        }

        var nextEmailToSend = emailsOfGroEntreLaunch[indexOfLastEmail + 1];
    ...
    private DateTimeOffset? GetNextExecutionTime(ContactEmailSchedule contactEmailSchedule, int retryDelay, EmailLog? lastEmailLog)
    {
        ...
        if (!string.IsNullOrEmpty(contactSchedule!.Cron))
        {
            ...
            var nextRunTimeForUser = expression.GetNextValidTimeAfter(new DateTimeOffset(lastRunTime, TimeSpan.Zero).ToOffset(-userToServerTimeZoneOffset).DateTime);

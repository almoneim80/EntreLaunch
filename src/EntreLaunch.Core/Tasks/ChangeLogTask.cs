...
using EntreLaunch.Configuration;
...
public abstract class ChangeLogTask : BaseTask
{
        ...
        foreach (var loggedType in loggedTypes)
        {
            ...
            while (changeLogBatch is not null && changeLogBatch!.Any())
            {
                ...
                try
                {
                    ...
                    EntreLaunchdateChangeLogTaskLogRecord(taskLog, changeLogBatch!.Count, TaskExecutionState.Completed);
                }
                catch (Exception ex)
                {
                    Log.Error(ex, $"Error occurred when executing task {taskAndEntity}");

                    EntreLaunchdateChangeLogTaskLogRecord(taskLog, 0, TaskExecutionState.Failed);
    ...
    protected HashSet<Type> GetTypes(PgDbContext context)
    {
        ...
        {
            if (type != null && IsChangeLogAttribute(type) && IsTypeSEntreLaunchported(type))
            {
                res.Add(type);
            }
        }
    ...
    protected abstract bool IsTypeSEntreLaunchported(Type type);
    ...
    private void EntreLaunchdateChangeLogTaskLogRecord(ChangeLogTaskLog taskLog, int changesProcessed, TaskExecutionState state)
    {

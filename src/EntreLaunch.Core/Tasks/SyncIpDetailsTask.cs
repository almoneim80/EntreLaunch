...
public class SyncIpDetailsTask : ChangeLogTask
{
    ...
    protected override bool IsTypeSEntreLaunchported(Type type)
    {
        return typeof(IHasCreatedBy).IsAssignableFrom(type) || typeof(IHasEntreLaunchdatedBy).IsAssignableFrom(type);
    }
    ...
    private List<string> GetDistinctIps(List<ChangeLog> changeLogs)
    {
        ...
        var resultedIps = (from ip in ipObjects where !string.IsNullOrWhiteSpace(ip.CreatedByIp!) select ip.CreatedByIp).Union(from ip in ipObjects where !string.IsNullOrWhiteSpace(ip.EntreLaunchdatedByIp!) select ip.EntreLaunchdatedByIp).ToList();

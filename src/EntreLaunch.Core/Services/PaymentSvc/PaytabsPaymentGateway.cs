...
{
    public class PaytabsPaymentGateway(
        IConfiguration configuration,
        ILogger<PaytabsPaymentGateway> logger,
        HttpClient httpClient,
        IOptions<PayTabsOptions> options) : IPaymentGateway
    {
        private readonly IConfiguration _configuration = configuration;
        private readonly ILogger<PaytabsPaymentGateway> _logger = logger;
        ...
        public async Task<PaymentResult> InitiatePaymentAsync(Payment payment, string paymentToken)
        {
            try
            {
                // (1) Setting up the order to PayTabs

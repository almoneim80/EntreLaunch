...
using EntreLaunch.Infrastructure.StaticContentInfra;
using EntreLaunch.Interfaces.StaticContentIntf;

namespace EntreLaunch.Services.StaticContentSvc
{
        ...
        PgDbContext dbContext, ILocalizationManager localization, ILogger<StaticContentService> logger, ICacheService cache) : IStaticContentService
    {
        ...
        /// <inheritdoc />
        public async Task<GeneralResult> SaveAsync(StaticContent content)
        {
            ...
            if (existing == null)
            {
                _dbContext.StaticContents.Add(content);
            }
            else
            {
                _dbContext.Entry(existing).CurrentValues.SetValues(content);

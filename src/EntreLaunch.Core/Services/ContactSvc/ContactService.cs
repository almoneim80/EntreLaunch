...
namespace EntreLaunch.Services.ContactSvc
{
    public class ContactService(
        ...
        ILocalizationManager localization) : IContactService
    {
        private readonly IDomainService domainService = domainService;
        private readonly IEmailSchedulingService emailSchedulingService = emailSchedulingService;
        private readonly IOptions<ApiSettingsConfig> apiSettingsConfig = apiSettingsConfig;
        private PgDbContext pgDbContext = pgDbContext;
        ...
        public async Task<GeneralResult<Contact>> FindOrCreate(string email, string language, int timezone)
        {
            try
            {
                ...
                return new GeneralResult<Contact>(true, _localizationManager.GetLocalizedString("ContactFoundOrCreated"), customer);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Failed to find or create contact");
                return new GeneralResult<Contact>(false, _localizationManager.GetLocalizedString("ErrorFindingOrCreatingContact"), null);
            }
        }

        /// <inheritdoc/>
        public async Task<GeneralResult> Subscribe(Contact contact, string groupName)
        {
            try
            {
                ...
                if (emailSchedule.Data == null)
                {
                    ...
                    return new GeneralResult(false, _localizationManager.GetLocalizedString("EmailScheduleNotFound"), null);
                }
                ...
                return new GeneralResult(true, _localizationManager.GetLocalizedString("ContactSubscribed"), null);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Failed to subscribe contact");
                return new GeneralResult(false, _localizationManager.GetLocalizedString("ErrorSubscribingContact"), null);
            }
        }
        ...
        public async Task<GeneralResult> Unsubscribe(string email, string reason, string source, DateTime createdAt, string? ip)
        {
            try
            {
                ...
                if (contact != null)
                {
                    ...
                    return new GeneralResult(true, _localizationManager.GetLocalizedString("ContactUnsubscribed"), null);
                }
                else
                {
                    _logger.LogError("Contact not found");
                    return new GeneralResult(false, _localizationManager.GetLocalizedString("ContactNotFound"), null);
                }
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Failed to unsubscribe contact");
                return new GeneralResult(false, _localizationManager.GetLocalizedString("ErrorUnsubscribingContact"), null);
            }
        }

...
{
    public class EmailVerifyService(PgDbContext pgDbContext, IDomainService domainService, IEmailValidationExternalService emailValidationExternalService, ILogger<EmailVerifyService> logger, ILocalizationManager localizationManager) : IEmailVerifyService
    {
        private readonly PgDbContext pgContext = pgDbContext;
        private readonly IDomainService domainService = domainService;
        private readonly IEmailValidationExternalService emailValidationExternalService = emailValidationExternalService;
        private readonly ILogger<EmailVerifyService> _logger = logger;
        ...
        public async Task<GeneralResult<Domain>> Verify(string email)
        {
            try
            {
                ...
                {
                    return new GeneralResult<Domain>(true, _localizationManager.GetLocalizedString("DomainVerifiedSuccessfully"), domain);
                }
                else
                {
            ...
            catch (Exception ex)
            {
                ...
                return new GeneralResult<Domain>(false, _localizationManager.GetLocalizedString("DomainVerificationFailed"), null);
            }
        }

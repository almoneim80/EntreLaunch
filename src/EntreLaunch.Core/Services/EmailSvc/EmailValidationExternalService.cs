...
namespace EntreLaunch.Services.EmailSvc
{
    public class EmailValidationExternalService : IEmailValidationExternalService
    {
        ...
        public EmailValidationExternalService(IOptions<EmailVerificationApiConfig> emailVerificationApiConfig, ILogger<EmailValidationExternalService> logger, ILocalizationManager localizationManager)
        {
        ...
        public async Task<GeneralResult<EmailVerifyInfoDto>> Validate(string email)
        {
            try
            {
                ...
                // Create an HTTP client and set up the header
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    return new GeneralResult<EmailVerifyInfoDto>(true, _localizationManager.GetLocalizedString("EmailIsValid"), emailVerify);
                }
                else
                {
            ...
            catch (Exception ex)
            {
                ...
                return new GeneralResult<EmailVerifyInfoDto>(false, _localizationManager.GetLocalizedString("EmailValidationError"), null);
            }
        }
    }
}

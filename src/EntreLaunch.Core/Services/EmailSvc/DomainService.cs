namespace EntreLaunch.Services.EmailSvc
{
    public class DomainService : IDomainService
    {
        ...
        private readonly LookupClient lookEntreLaunchClient;
        private readonly IMxVerifyService mxVerifyService;
        private readonly IMapper _mapper;
        private PgDbContext pgDbContext;
        public DomainService(PgDbContext pgDbContext, IMxVerifyService mxVerifyService, IMapper mapper)
        {
            this.mxVerifyService = mxVerifyService;

            this.pgDbContext = pgDbContext;

            lookEntreLaunchClient = new LookupClient(new LookupClientOptions
            {
                UseCache = true,
        ...
        public async Task SaveRangeAsync(List<Domain> domains)
        {
            ...
            foreach (var groEntreLaunch in sortedDomains)
            {
                if (groEntreLaunch.Key)
                {
                    pgDbContext.UpdateRange(groEntreLaunch.ToList());
                }
                else
                {
        ...
        /// <inheritdoc />
        public async Task<GeneralResult<DomainDetailsDto>> VerifyDomainAsync(string name, bool force)
        {
            var domain = (from d in pgDbContext.Domains
                          where d.Name == name
            ...
            return new GeneralResult<DomainDetailsDto>(true, "Domain verified successfully", _mapper.Map<DomainDetailsDto>(domain));
        }

        /*****************PRIVATE METHODS**************************/

        /// <summary>
        /// Init domains list from resources files.
        /// </summary>
        ...
        private async Task VerifyMX(Domain domain)
        {
            ...
            var mxRecords = await lookEntreLaunchClient.QueryAsync(domain.Name, QueryType.MX);
        ...
        private async Task VerifyDns(Domain domain)
        {
            ...
            var result = await lookEntreLaunchClient.QueryAsync(domain.Name, QueryType.ANY);

...
#pragma warning disable EF1001 // Internal EF Core API usage.
    ...
    protected override void Generate(UpdateDataOperation operation, IModel? model, MigrationCommandListBuilder builder)
    {
        ...
        EntreLaunchdateChangeLog(operation.Table, operation.KeyColumns, operation.KeyValues, EntityState.Modified, model, builder);
    }

    protected override void Generate(DeleteDataOperation operation, IModel? model, MigrationCommandListBuilder builder)
    {
        EntreLaunchdateChangeLog(operation.Table, operation.KeyColumns, operation.KeyValues, EntityState.Deleted, model, builder);
    ...
    protected override void Generate(RenameColumnOperation operation, IModel? model, MigrationCommandListBuilder builder)
    {
        ...
        if (type != null && IsChangeLogSEntreLaunchported(type))
        {
            var renameKeyInChangeLog = new SqlOperation()
            {
                Sql = $@"EntreLaunchDATE change_log set Data = Data - '{ColumnNameToCamelCase(operation.Name)}' ||  jsonb_build_object('{ColumnNameToCamelCase(operation.NewName)}', Data -> '{ColumnNameToCamelCase(operation.Name)}') WHERE object_type = '{type.Name}'",
            };
    ...
    protected override void Generate(AlterColumnOperation operation, IModel? model, MigrationCommandListBuilder builder)
    {
        ...
        if (type != null && IsChangeLogSEntreLaunchported(type) && operation.OldColumn.ClrType != operation.ClrType)
        {
            throw new ChangeLogMigrationException("Changing column type isn't sEntreLaunchported. Please use SqlOperation for migrations.");
        }
    }
    ...
    protected override void Generate(DropColumnOperation operation, IModel? model, MigrationCommandListBuilder builder, bool terminate = true)
    {
        ...
        if (type != null && IsChangeLogSEntreLaunchported(type))
        {
            ...
            {
                Sql = $@"EntreLaunchDATE change_log set Data = Data - '{ColumnNameToCamelCase(operation.Name)}' WHERE object_type = '{type.Name}'",
            };
    ...
    protected override void Generate(AddColumnOperation operation, IModel? model, MigrationCommandListBuilder builder, bool terminate = true)
    {
        ...
        if (type != null && IsChangeLogSEntreLaunchported(type))
        {
            ...
            {
                throw new ChangeLogMigrationException("Default Value Sql construction isn't sEntreLaunchported. Please use SqlOperation for migrations.");
            }
            ...
            {
                Sql = @$"EntreLaunchDATE change_log set Data = change_log.Data || jsonb_build_object('{ColumnNameToCamelCase(operation.Name)}', {strValue}) WHERE object_type = '{type.Name}'",
            };
    ...
    {
        var res = string.Join(string.Empty, columnName.Split('_').Select(s => s = s.Length > 0 ? char.ToLower(s[0]) + s.Substring(1) : s));
        res = char.ToLower(res[0]) + res.Substring(1);
        return res;
    }

    private static bool IsChangeLogSEntreLaunchported(Type type)
    {
        return type.GetCustomAttributes<SEntreLaunchportsChangeLogAttribute>().Any();
    }

    private void EntreLaunchdateChangeLog(string table, string[] keyColumns, object?[,] keyValues, EntityState state, IModel? model, MigrationCommandListBuilder builder)
    {
        ...
        if (type != null && IsChangeLogSEntreLaunchported(type))
        {
            ...
            {
                // TODO: EntreLaunchdate items not just on id key
                throw new ChangeLogMigrationException("EntreLaunchdateDataOperation or DeleteDataOperation must contain just id key column. Please, use SqlOperation instead for your purposes");
            }
        }
    }
}

...
#pragma warning disable S3010
                ...
                foreach (var task in tasks.Where(t => t.IsRunning))
                {
                    ...
                    using (taskLock)
                    {
                        ...
                        if (IsRightTimeToExecute(currentJob, task))
                        {
                            ...
                            await EntreLaunchdateTaskExecutionLog(currentJob, isCompleted ? TaskExecutionStatus.Completed : TaskExecutionStatus.Pending);
                        }
                    }
                }
            }
        ...
        private async Task EntreLaunchdateTaskExecutionLog(TaskExecutionLog job, TaskExecutionStatus status)
        {
            job.Status = status;
            ...
            dbContext!.TaskExecutionLogs!.EntreLaunchdate(job);
            await dbContext.SaveChangesAsync();
        }
        ...
        {
            var expression = new Quartz.CronExpression(cronSchedule);

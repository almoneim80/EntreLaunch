...
{
    /// <summary>
    ///  Execute live queries using Elasticsearch.
    /// </summary>
        ...
        where T : BaseEntityWithId
    {
        ...
        public ESQueryProvider(ElasticClient elasticClient, QueryModelBuilder<T> queryBuilder, string indexPrefix)
        {
            ...
            elasticClient.Indices.EntreLaunchdateSettings(indexName, s => s.IndexSettings(i => i.Setting(EntreLaunchdatableIndexSettings.MaxResultWindow, maxResultWindow)));
        }
        ...
        public async Task<QueryResult<T>> GetResult()
        {
            ...
            try
            {
                ...
                if (queryBuilder.SelectData.IsSelect)
                {
                    ...
                    sr.Source = new Nest.SourceFilter { Includes = fields.ToArray(), };
                }

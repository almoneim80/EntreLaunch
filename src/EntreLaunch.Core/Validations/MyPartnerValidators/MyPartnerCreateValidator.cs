...
{
    public class MyPartnerCreateValidator : AbstractValidator<MyPartnerCreateDto>
    {
        public MyPartnerCreateValidator(ILocalizationManager localizationManager)
        {
            ...
            RuleFor(x => x.Status)
                .IsInEnum()
                .WithMessage(localizationManager.GetLocalizedString("InvalidPartnerStatus"));

            RuleFor(x => x.Attachments)
                .Must(lst => lst != null && lst.Any()).When(x => x.Attachments != null)
                .WithMessage(localizationManager.GetLocalizedString("AttachmentsNotEmpty"))
                .ForEach(att => att.SetValidator(new MyPartnerAttachmentCreateValidator(localizationManager)));
        }
    }
}

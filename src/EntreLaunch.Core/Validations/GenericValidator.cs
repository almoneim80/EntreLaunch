...
public static class GenericValidator
{
    ...
    public static IRuleBuilderOptions<T, DateTimeOffset?> MustBeLaterThan<T>(
    this IRuleBuilder<T, DateTimeOffset?> ruleBuilder,
    Func<T, DateTimeOffset?> otherPropertyFunc,
    Func<T, string> otherDateDisplayFunc)
    {
        return ruleBuilder
            ...
            .WithMessage((dto, value) =>
                $"Date must be later than {otherDateDisplayFunc(dto)}.");
    }
        ...
        bool mustBeFuture = false)
    {
        return ruleBuilder
            .Must(date =>
            {
                ...
                if (mustBePast && date >= utcNow) return false;
                if (mustBeFuture && date <= utcNow) return false;
            ...
            .WithMessage(_ =>
            {
                ...
                if (minDate.HasValue && maxDate.HasValue)
                    return $"Date must be between {minDate.Value:d} and {maxDate.Value:d}.";

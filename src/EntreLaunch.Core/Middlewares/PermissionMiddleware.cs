        ...
        public async Task Invoke(HttpContext context, IPermissionService permissionService)
        {
            ...
            if (!_cache.TryGetValue(cacheKey, out HashSet<string>? permissions))
            {
                ...
                if (!result.IsSuccess || result.Data == null)
                {
                    ...
                    var message = _localization?.GetLocalizedString("MissingPermission") ?? "Permission denied.";

                    var errorResult = new GeneralResult
                    {
                        IsSuccess = false,
                        Message = message,
                        Data = new { Permission = requiredPermission },
                        ErrorType = Enums.ErrorType.PermissionDenied
                    };
            ...
            if (!(permissions?.Contains(requiredPermission) ?? false))
            {
                context.Response.StatusCode = StatusCodes.Status403Forbidden;
                context.Response.ContentType = "application/json";
                var message = _localization?.GetLocalizedString("MissingPermission")?.Replace("{requiredPermission}", requiredPermission) ?? "Permission denied.";

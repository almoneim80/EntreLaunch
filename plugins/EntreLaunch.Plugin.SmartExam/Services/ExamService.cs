...
namespace EntreLaunch.Plugin.SmartExam.Services
{
    public class ExamService : IExamService
    {
        ...
        public async Task<string> GenerateQuestionsAsync(AIRequestDataDto aIRequestDataDto)
        {
            ...
            // Setting up the POST message and attaching the JSON to the content
        ...
        public async Task<List<string>> UploadFilesToDify(List<IFormFile> files, string userId)
        {
            if (files == null || files.Count == 0)
                throw new Exception("❌ Error: No files uploaded.");

            var uploadUrl = "https://api.dify.ai/v1/files/upload";
            ...
            foreach (var file in files)
            {
                ...
                if (!response.IsSuccessStatusCode)
                    throw new Exception($"❌ Upload Error: {response.StatusCode}, Details: {responseContent}");

namespace EntreLaunch.Plugin.SmartExam.Controllers
{
    ...
    public class SmartExamController : ControllerBase
    {
        ...
        /// <summary>
        /// Upload a file and generate exam from its content.
        /// </summary>
        [HttpPost("exam/generate-exam-from-documents")]
        [Consumes("multipart/form-data")]
        public async Task<IActionResult> UploadFilesToAI(List<IFormFile> files, [FromForm] GenerateQuestionsRequestDto requestDto)
        {
            if (files == null || files.Count == 0)
                return BadRequest("❌ Error: No files uploaded.");

            if(files.Count > 5)
            {
                return BadRequest("❌ Error: You can upload a maximum of 5 files.");
            }

            try
            {
                var fileIds = await _examService.UploadFilesToDify(files, "123");
